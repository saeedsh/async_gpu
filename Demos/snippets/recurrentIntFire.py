...
def make_network():
    ...
    for item in syns.vec:
        ...
        sh.synapse.delay = delayMin + \
            (delayMax - delayMin) * nprand.rand(len(sh.synapse))
        # sh.synapse.delay = [ (delayMin + random.random() * (delayMax -
        # delayMin ) for r in range( len( sh.synapse ) ) ]
